...
class BfsBoard
  def proccess(matrix, show_step = false)
    ...
    list.each do |item|
      loop do
        shot_step = find_step(item)
        if shot_step && !has_one_element?(item.matrix)
          matrix_before_iteration(item.matrix, shot_step) if show_step
          figure = build_figure(shot_step, item.matrix)
          cloned_matrix = item.matrix.clone
          matrix = make_nil(figure, cloned_matrix)
          matrix = resize(matrix)
          node = Node.new({ matrix: matrix, shot_step: shot_step, parent: item, figure: figure })
