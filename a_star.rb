...
class AStar
  def proccess(matrix, show_step = false)
    root = Node.new({ matrix: matrix.clone, shot_step: [] })
    result = nil
    list = [root]

    loop do
      current_node = list.sort! { |x,y| y.figure.length <=> x.figure.length }.shift
      if !has_one_element?(current_node.matrix)
        current_node.figures = all_figures(current_node)
        current_node.figures.each do |figure|
          ...
          matrix_after_iteration(node.matrix) if show_step
          result = node if solved?(node.matrix)
          break if result
